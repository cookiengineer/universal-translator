...
func (t *TextView) SetWrapMode(mode int) {
	C.gtk_text_view_set_wrap_mode((*C.GtkTextView)(unsafe.Pointer(t.widget)), C.GtkWrapMode(mode))
}
...
func (t *TextView) SetText(text string) {
	...
	buffer := C.gtk_text_view_get_buffer((*C.GtkTextView)(unsafe.Pointer(t.widget)))
	C.gtk_text_buffer_set_text(buffer, cText, C.int(-1))
}

func (t *TextView) GetText() string {
	buffer := C.gtk_text_view_get_buffer((*C.GtkTextView)(unsafe.Pointer(t.widget)))

	var start C.GtkTextIter
	var end C.GtkTextIter
	C.gtk_text_buffer_get_bounds(buffer, &start, &end)

	text := C.gtk_text_buffer_get_text(buffer, &start, &end, C.FALSE)
